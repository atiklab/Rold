...
int vspeed=50;
int calSpeed = 70;   // tune value motors will run while auto calibration sweeping turn (0-255)
int turnSpeed = 70;  // tune value motors will run while turning (0-255)
int turnSpeedSlow = 50;  // tune value motors will run as they slow down from turning cycle to avoid overrun (0-255)
...
void setup()
{
  ...
  analogWrite(pwm_b, turnSpeedSlow); 
        m1Speed=turnSpeedSlow;
        m2Speed=turnSpeedSlow;
...
void follow_line()  //follow the line
{
  ...
  while(1)
  {
    ...
    if(sensorValues[1] < lv && sensorValues[2] < lv && sensorValues[3] < lv && sensorValues[4] < lv && sensorValues[5] < lv && sensorValues[6] < lv)
    {
    ...
    else if(sensorValues[0] > mv || sensorValues[7] > mv)
    {
...
void turn(char dir)
{
  switch(dir)
  {
    ...
    case 'L':    
      ...
      while (sensorValues[6] <mv)  // wait for outer most sensor to find the line
      {
        line_position = qtrrc.readLine(sensorValues);
      }
    ...
    case 'B':		
      ...
      while (sensorValues[1] <mv)  // wait for outer most sensor to find the line
      {
        line_position = qtrrc.readLine(sensorValues);
      }
...
void MazeSolve()
{
  ...
  while(1)
  {
    ...
    digitalWrite(dir_a, LOW);  
    analogWrite(pwm_a, vspeed);
    digitalWrite(dir_b, LOW);  
    analogWrite(pwm_b, vspeed);   
        m1Speed=vspeed;
        m2Speed=vspeed;
        mway('f'); 
    delay(10); 
    ...
    if(sensorValues[0] > mv)
    found_right = 1;
    if(sensorValues[7] > mv)
    found_left = 1;
    ...
    line_position = qtrrc.readLine(sensorValues);
    if(sensorValues[1] > mv || sensorValues[2] > mv || sensorValues[3] > mv || sensorValues[4] > mv || sensorValues[5] > mv || sensorValues[6] > mv)
    found_straight = 1;

    // Check for the ending spot.
    // If all six middle sensors are on dark black, we have
    // solved the maze.
    if(sensorValues[1] > hv && sensorValues[2] > hv && sensorValues[3] > hv && sensorValues[4] > hv && sensorValues[5] > hv && sensorValues[6] > hv)
	break;
  ...
  while(1)
  {
    ...
    line_position = qtrrc.readLine(sensorValues);
    while(sensorValues[1] > mv && sensorValues[2] > mv && sensorValues[3] > mv && sensorValues[4] > mv && sensorValues[5] > mv && sensorValues[6] > mv)
    {
      line_position = qtrrc.readLine(sensorValues);
      delay(50);
    }
    ...
    {
      line_position = qtrrc.readLine(sensorValues);
      if(sensorValues[2] > mv || sensorValues[3] > mv || sensorValues[4] > mv || sensorValues[5] > mv && sensorValues[0] < mv && sensorValues[1] < mv && sensorValues[6] < mv && sensorValues[7] < mv)
      break;
      delay(50);
    }
