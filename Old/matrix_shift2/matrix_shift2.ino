...
int pauseDelay = 1500;  //microseconds to leave each row  on before moving to the next

char requestString[] = " MD ATIKUR RABBI SHAWON ";  //The string to display
...
//Pin Definitions
int rowA[] = { 14,15,16,17,18,19,20};          //An Array defining which pin each row is attached to
                                         //(rows are common anode (drive LOW))
int colA[] = {2,3,4,5,6,7,8,9};  //An Array defining which pin each column is attached to
                                         //(columns are common cathode (drive HIGH))
...
void showSprite(int speed2){
 for(int iii = 0; iii < speed2; iii++){                 //show the current frame speed2 times
  for(int column = 0; column < 8; column++){            //iterate through each column
   ...
   delayMicroseconds(pauseDelay);                       //leave the column on for pauseDelay microseconds (too LOW a delay causes flicker)
  } 
 }
}
