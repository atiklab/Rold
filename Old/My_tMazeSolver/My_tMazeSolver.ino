...
#define EMITTER_PIN   A8  // emitter control pin not used.  If added, replace QTR_NO_EMITTER_PIN with pin#
...
int calSpeed = 50;   // tune value motors will run while auto calibration sweeping turn (0-255)
int turnSpeed = 50;  // tune value motors will run while turning (0-255)
...
float lastError=0;
float PV =0 ;
float kp = .1;  // tune value in follow_line() function
  ...
  for (int i = 0; i <= 100; i++)  // begin calibration cycle to last about 2.5 seconds (100*25ms/call)
  {
    ...
    if (i==0 || i==60)  // slow sweeping turn right to pass sensors over line
    {
       ...
       analogWrite(pwm_b, calSpeed);   
        analogWrite(lf, calSpeed);
        analogWrite(lb, 0); 
        analogWrite(rf, 0);
        analogWrite(rb, calSpeed);
    ...
    else if (i==20 || i==100)  // slow sweeping turn left to pass sensors over line
    {
       ...
       analogWrite(pwm_b, calSpeed);
       analogWrite(lf, 0);
        analogWrite(lb, calSpeed); 
        analogWrite(rf, calSpeed);
        analogWrite(rb, 0);   
    }
  ...
  while(1)
  {
    ...
    switch(line_position)
    {
      ...
      default:
        ...
        analogWrite(lf, m2Speed);
        analogWrite(lb, 0); 
        analogWrite(rf, m1Speed);
        analogWrite(rb, 0);
        break;
    }
