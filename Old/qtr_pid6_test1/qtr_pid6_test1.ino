...
int rf = 9; 
int rb = 10; 
int  SPEED=70,MSPEED=160;
int sec=0;
int cr=300;

QTRSensorsRC qtrrc((unsigned char[]) {A1, A2, A3, A4, A5,A6} ,NUM_SENSORS, TIMEOUT, EMITTER_PIN); // sensor connected through analog pins A0 - A5 i.e. digital pins 14-19
...
if((position!=0)&&(position!=5000)&&(sensorValues2[0]<cr))
   {
     mway2(leftMotorSpeed,rightMotorSpeed);
   }
     else if ((sec!=1)&&((position==0)||(position==5000)))//&&(position2<1500))
  {
    SPEED= MSPEED;
   mway('L'); 
  }
  
  else if ((position2==1000)&&((position==0)||(position==5000))&&(sec==1))
  {
    SPEED= MSPEED;
   mway('R'); 
  }
